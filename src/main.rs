...
/// is built and verified. Returns (status_list, prove_time, verify_time).
///
/// Signing is deliberately untimed. In a deployment the `t` signatures come one
/// each from `t` separate machines; producing them all in one loop and timing it
/// would report a committee's total work as if one node paid it. The cost of a
/// member's round is measured on its own, in `src/bin/signer.rs`.
///
...
fn main() {
    ...
    // alone, `setup_prover` adds the arena and the DFT twiddles. The modules are
    // built after keygen and their setup calls then land as no-ops: every one of
    // leanVM's is idempotent (`OnceLock`, `Once`, `mallopt`, an early return once
    // the twiddle table is large enough).
    println!("setup...");
    let t_v = Instant::now();
    setup_verifier();
    ...
    //
    //    Built **slot-consistent** on purpose: signed at the slot the *inflated*
    //    version derives to, so check 3 passes and check 2 is the only thing
    //    between this record and acceptance. Signing at an unrelated slot would
    //    let check 3 reject it first, and the test would stop proving anything
    //    about check 2.
    //
    //    `slot = genesis + version` also bounds the lie: the attacker needs a key
    //    covering that slot, so KEY_SLOTS is the largest inflation available.
